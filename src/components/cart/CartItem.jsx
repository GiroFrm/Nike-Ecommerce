...
const CartItem = ( {item: { id, title, text, img, color, shadow,
   price, cartQuantity }}) => {
  const dispatch = useDispatch();

  const onRemoveItem = () => {
    dispatch(setRemoveItemFromCart({ id, title, text, img, color, shadow,
        price, cartQuantity }))
  }

  const onIncreaseItemQTY = () => {
    dispatch(setIncreaseItemQTY({ id, title, text, img, color, shadow,
        price, cartQuantity }))
  }
  
  const onDecreaseItemQTY = () => {
    dispatch(setDecreaseItemQTY({ id, title, text, img, color, shadow,
        price, cartQuantity }))
  } 
            ...
            <div className='grid items-center gap-4'>
                ...
                <div className='flex items-center justify-around w-full'>
                    <button type='button' onClick={onDecreaseItemQTY}
                     className='bg-theme-cart rounded w-6 h-6 lg:w-5 lg:h-5 flex items-center 
                  ...
                  <div className='bg-theme-cart rounded
                   text-white font-medium lg:text-xs w-7 h-6 lg:h-5 lg:w-6 flex 
                  ...
                  <button type='button' onClick={onIncreaseItemQTY}
                     ...
                     justify-center active:scale-90'
                    ><PlusIcon
                     className='w-5 h-5w-5 h-5 lg:w-4 lg:h-4 text-white 
                     storke-[2]'
                    />
                  </button>
